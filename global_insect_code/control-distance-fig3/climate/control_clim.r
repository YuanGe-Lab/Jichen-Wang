...
max1 <-  max(geocld[!is.na(geocld)])
geocld[is.na(geocld)] <- 10

seq1 <- seq(min(geocld),max1,(max1 / 400))
...
#  geom_vline(xintercept = 0.08, color = "red",linetype = 2) +
  labs( x = "Climatic distance", y = "Spearman correlation")

p1
ggsave(p1, file="geocl_step_new.pdf", width=6, height=2)
p2 <- p1 + xlim(c(0,0.25))
ggsave(p2, file="geocl_step_new2.pdf", width=5, height=2)
